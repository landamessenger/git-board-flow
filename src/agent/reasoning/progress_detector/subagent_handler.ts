...
export class SubagentHandler {
   ...
   */
  static async detectProgressWithSubAgents(
    agent: Agent,
    repositoryFiles: Map<string, string>,
    options: ProgressDetectionOptions,
    userPrompt: string
  ...
  private static combineSubagentResults(
    ...
    const allProgressReports: Array<{ progress: number; summary: string }> = [];
    ...
    logInfo(`✅ Combined results: Average progress ${averageProgress}% across ${results.length} subagents`);
    logInfo(`   Individual progress reports:`);
    allProgressReports.forEach((r, i) => {
      logInfo(`   - Subagent ${i + 1}: ${r.progress}%`);
    });
