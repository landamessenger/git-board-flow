...
describe('FilePartitioner', () => {
  describe('partitionFilesByDirectory', () => {
    ...
    it('should handle more partitions than files', () => {
    ...
    it('should balance files across partitions when directories have different sizes', () => {
      const files = [
        ...Array.from({ length: 20 }, (_, i) => `src/agent/file${i}.ts`),
        ...Array.from({ length: 5 }, (_, i) => `src/utils/file${i}.ts`),
        ...Array.from({ length: 3 }, (_, i) => `test/file${i}.ts`)
      ];
      ...
      const partitionSizes = result.map(p => p.length);
      const maxSize = Math.max(...partitionSizes);
      const minSize = Math.min(...partitionSizes);
      
      // Should try to balance, but may not be perfect due to directory grouping
      // With 20 files in one directory, the difference can be larger
      expect(maxSize - minSize).toBeLessThanOrEqual(25);
    });
  });
});
