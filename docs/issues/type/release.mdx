...
1. A release branch is created from `development-branch`
2. A feature branch is created from the release branch to implement any remaining changes
3. The needed changes are applied on the feature branch
4. The feature branch is merged back into the release branch via Pull Request
5. A new version is deployed from the release branch by adding the `deploy-label` to the issue
6. After successful deployment, the release branch is merged:
