...
| **`ai_cache_action`** | — | Runs vector indexing / AI cache update for the repository (codebase analysis and embeddings). No issue required. |
| **`ai_cache_local_action`** | — | Same as `ai_cache_action` but for local/self-hosted runner (e.g. with local AI container). No issue required. |
...
Single actions that **throw an error** if the last step fails: `ai_cache_action`, `publish_github_action`, `create_release`, `deployed_action`, `create_tag`. This lets the workflow fail the job when the action does not succeed.
...
| **Vector / AI cache** | Single actions `ai_cache_action`, `ai_cache_local_action` | Codebase analysis and embedding generation for semantic search. |
...
- **Supabase** (optional): `supabase-url` and `supabase-key` for optional backend features.
