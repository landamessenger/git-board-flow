...
|--------------------|----------------|-------------|
| **`check_progress_action`** | `single-action-issue` | Uses OpenCode Plan to analyze the branch diff vs the issue description and posts a **progress percentage** (e.g. "Progress: 30%") and summary as a comment on the issue. |
...
|--------|----------------|-------------|
| **Check progress** | Single action `check_progress_action` | OpenCode Plan agent compares issue description vs branch diff and reports a percentage and summary. |
| **Think / reasoning** | Single action `think_action` | Deep code analysis and change proposals (OpenCode Plan agent). |
| **AI PR description** | Pull request pipeline | Generates or updates PR description from issue and diff (OpenCode Plan agent). |
