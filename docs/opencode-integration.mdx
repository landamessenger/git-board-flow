...
Example (action starts and stops OpenCode for you; no separate server needed):
...
3. **AI analysis (OpenCode)** – A **ProgressDetector** agent runs against your OpenCode server. It receives the issue description, list of changed files, and (when used) file contents. The agent compares what was requested in the issue vs what is implemented in the diff and calls the **report_progress** tool once with `progress` (0–100) and `summary` (short explanation).
