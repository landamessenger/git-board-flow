...
description: Step-by-step guide to test check-progress, detect-errors, and recommend-steps locally using the CLI
---

# Testing OpenCode Plan Locally

This guide walks you through testing the OpenCode Plan flows (`check-progress`, `detect-errors`, `recommend-steps`) locally using the `giik` CLI, without running GitHub Actions.
...
- `--opencode-model <model>` – Override model (e.g. `opencode/kimi-k2.5`, `anthropic/claude-3-5-sonnet`)
- `-d, --debug` – Verbose logs

### Detect errors

Detects potential errors in the branch vs base using OpenCode Plan:

```bash
node build/cli/index.js detect-errors -i <issue-number> -t <PAT>
```
